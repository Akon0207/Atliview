...
(function (global, factory) {
  ...
  var css = "ul{margin:0;padding:0}li{list-style-type:none}.rolldate-container{font-size:20px;color:#333;text-align:center}.rolldate-container header{position:relative;line-height:60px;font-size:18px;border-bottom:1px solid #e0e0e0}.rolldate-container .rolldate-mask{position:fixed;width:100%;height:100%;top:0;left:0;z-index:999;background-color:rgba(37,38,45,.4)}.rolldate-container .rolldate-panel{position:fixed;bottom:60px;left:0;width:100%;height:273px;z-index:1000;background:#fff;-webkit-transition:all .3s ease-in-out;-o-transition:all .3s ease-in-out;transition:all .3s ease-in-out;-webkit-transform:translate3d(0,273px,0);transform:translate3d(0,273px,0)}.rolldate-container .rolldate-btn{position:absolute;left:0;top:0;height:100%;padding:0 15px;color:#666;font-size:16px;cursor:pointer;-webkit-tap-highlight-color:transparent}.rolldate-container.wx .rolldate-btn{height:150%}.rolldate-container .rolldate-confirm{left:auto;right:0;color:#007bff}.rolldate-container .rolldate-content{position:relative;top:0px}.rolldate-container .rolldate-wrapper{display:-webkit-box;display:-webkit-flex;display:-ms-flexbox;display:flex}.rolldate-container .rolldate-wrapper>div{-webkit-box-flex:1;-webkit-flex:1;-ms-flex:1;flex:1;height:173px;line-height:36px;overflow:hidden;-webkit-flex-basis:-8e;-ms-flex-preferred-size:-8e;flex-basis:-8e;width:1%}.rolldate-container .rolldate-wrapper ul{margin-top:68px}.rolldate-container .rolldate-wrapper li{height:36px}.rolldate-container .rolldate-dim{position:absolute;left:0;top:0;width:100%;height:68px;background:-o-linear-gradient(bottom,hsla(0,0%,100%,.4),hsla(0,0%,100%,.8));background:-webkit-gradient(linear, left bottom, left top, from(hsla(0, 0%, 100%, 0.4)), to(hsla(0, 0%, 100%, 0.8)));background:-o-linear-gradient(bottom, hsla(0, 0%, 100%, 0.4), hsla(0, 0%, 100%, 0.8));background:linear-gradient(0deg,hsla(0,0%,100%,.4),hsla(0,0%,100%,.8));pointer-events:none;-webkit-transform:translateZ(0);transform:translateZ(0);z-index:10}.rolldate-container .mask-top{border-bottom:1px solid #ebebeb}.rolldate-container .mask-bottom{top:auto;bottom:1px;border-top:1px solid #ebebeb}.rolldate-container .fadeIn{-webkit-transform:translateZ(0);transform:translateZ(0)}.rolldate-container .fadeOut{-webkit-transform:translate3d(0,273px,0);transform:translate3d(0,273px,0)}@media screen and (max-width:414px){.rolldate-container{font-size:18px}}@media screen and (max-width:320px){.rolldate-container{font-size:15px}}.rolldate-container span{display:table-cell;text-align:center;color:#666}.date-title{width:100%;display:table}";
  styleInject(css);
  ...
  Rolldate.prototype = {
      ...
      createUI: function createUI() {
          var _this = this,
              ...
              lang = config.lang,
			  titleSpan = '';
            console.log("el.value ==== ",config.el);
          for (var i = 0; i < len; i++) {
              var f = FormatArr[i],
                  domMndex = 0;
              ...
              if (f == 'YYYY') {
				  ...
				  titleSpan += '<span>' + lang.month + '</span>';
                  for (var k = 1; k <= 180; k++) {
					  var k0 = (k - 1) % 12 + 1;
                      itemClass = (k - 84)  == date.getMonth() + 1 ? 'active' : '';

                      ul += '<li class="wheel-item ' + itemClass + '" data-index="' + domMndex + '">' + (k0 < 10 ? '0' + k0 : k0) + '</li>';
                      domMndex++;
                  }
              } else if (f == 'DD') {
				  titleSpan += '<span>' + lang.day + '</span>';
                  var day = _this.bissextile(date.getFullYear(), date.getMonth() + 1);
                  for (var l = 1; l <= day * 7 ; l++) {
					  var l0 = (l - 1) % day + 1;
                      itemClass = (l - day *3) == date.getDate() ? 'active' : '';

                      ul += '<li class="wheel-item ' + itemClass + '" data-index="' + domMndex + '">' + (l0 < 10 ? '0' + l0 : l0) + '</li>';
                      domMndex++;
                  }
              } else if (f == 'hh') {
				  titleSpan += '<span>' + lang.hour + '</span>';
                  for (var z = 0; z <= 167; z++) {
					  var m = z % 24;
                      itemClass = (z-72) == date.getHours() ? 'active' : '';

                      ul += '<li class="wheel-item ' + itemClass + '" data-index="' + (domMndex) + '">' + (m < 10 ? '0' + m : m) + '</li>';
                      domMndex++;
                  }
              } else if (f == 'mm') {
				  titleSpan += '<span>' + lang.min + '</span>';
                  for (var n = 0; n <= 179; n += config.minStep) {
					  var n0 = n % 60;
                      itemClass = (n - 60) == date.getMinutes() ? 'active' : '';

                      ul += '<li class="wheel-item ' + itemClass + '" data-index="' + domMndex + '">' + (n0 < 10 ? '0' + n0 : n0) + '</li>';
                      domMndex++;
                  }
              } else if (f == 'ss') {
				  titleSpan += '<span>' + lang.sec + '</span>';
                  for (var o = 0; o <= 179; o++) {
					  var o0 = o % 60;
                      itemClass = (o - 60) == date.getSeconds() ? 'active' : '';

                      ul += '<li class="wheel-item ' + itemClass + '" data-index="' + domMndex + '">' + (o0 < 10 ? '0' + o0 : o0) + '</li>';
                      domMndex++;
                  }
              } else if (f == 'fps') {
                  titleSpan += '<span>' + lang.fps + '</span>';
                  console.log("_this.$(config.el).innerHTML:==="+_this.$(config.el).innerHTML);
                  for(var z = 5; z<=25; z++) {
                    itemClass = z == _this.$(config.el).innerHTML.split(" ")[0] ? 'active' : '';
                    ul += '<li class="wheel-item ' + itemClass + '" data-index="' + domMndex + '">' + z + '</li>';
                    domMndex++;
                  }
              } else if (f == 'zone') {
                titleSpan += '<span>' + lang.zone + '</span>';
                console.log("_this.$(config.el).innerHTML:===",_this.$(config.el).innerHTML);
                for(var i=-12;i<15;i++){
                  if(_this.$(config.el).innerHTML=="自动" || _this.$(config.el).innerHTML=="Auto" || _this.$(config.el).innerHTML==""){
                    var currentTimezone = -(new Date().getTimezoneOffset()/60);
                    itemClass = i==currentTimezone ? "active":"";
                  }else{
                    var timezone = parseInt(_this.$(config.el).innerHTML);
                    itemClass = i==timezone?'active':'';
                  }
                  ul += '<li class="wheel-item ' + itemClass + '" data-index="' + domMndex + '">' + (i>0?'+'+i:i) + '</li>';
                  domMndex++;
                }
              }
              ul += '</ul></div>';
          }
          if(FormatArr.includes("zone")){
            var $html = '<div class="rolldate-mask"></div>\n            <div class="rolldate-panel">\n                <ul class="mode-select"><li class="mode-item on">'+(language=='en'?'Auto':'自动')+'</li><li class="mode-item">'+(language=='en'?'Manual':'手动')+'</li></ul><header>\n                    <span class="rolldate-btn rolldate-cancel">' + lang.cancel + '</span>\n                    ' + lang.title + '\n                    <span class="rolldate-btn rolldate-confirm">' + lang.confirm + '</span>\n                </header>\n                <div class="date-title">' + titleSpan + '</div><section class="rolldate-content">\n                    <div class="rolldate-select-mask"></div><div class="rolldate-dim mask-top"></div>\n                    <div class="rolldate-dim mask-bottom"></div>\n                    <div class="rolldate-wrapper">\n                        ' + ul + '\n                    </div>\n                </section>\n            </div>';
          }else{
          ...
          console.log("mode-select:"+mode_select);
          if(FormatArr.includes("zone")){
            // getJSON("/setting",function(data){
            //   var timezone_mode = data.timezone_mode;
            //   if(timezone_mode == "manual")var timezone = data.timezone;
            //   if(timezone_mode == "auto"){
            //     $(".mode-select li").eq(0).addClass("on").siblings().removeClass("on");
            //     $(".rolldate-select-mask").show();
            //   }else{
            //     $(".mode-select li").eq(1).addClass("on").siblings().removeClass("on");
            //     $(".rolldate-select-mask").hide();
            //   }
            // },function(){
            //     $(".mode-select li").eq(0).addClass("on").siblings().removeClass("on");
            //     $(".rolldate-select-mask").show();
            ...
            $(".mode-select li").each(function(i){
          ...
          console.log(this);
                  ...
                  if ([domId['YYYY'], domId['MM']].indexOf(that.wrapper.id) != -1 && _this.scroll['YYYY'] && _this.scroll['MM'] && _this.scroll['DD']) {
                      var prevDay = _this.getSelected(_this.scroll['DD']),
                          ...
                          li = '';
					  var last_day_length =  _this.$('#' + domId['DD'] + ' li', 1).length;
                      if (_day != last_day_length) {

                          //for (var _l = 1; _l <= _day; _l++) {
						  var last_day = parseInt(_this.getSelected(_this.scroll['DD']));
						  var day_index;
						  for (var l = 1; l <= _day * 7 ; l++) {
					          var _l = (l - 1) % _day + 1;
							  if(last_day > _day) last_day = _day;
							  if((l - _day *3) == last_day) {
								  	day_index = l - 1;
							  }
                              li += '<li class="wheel-item" data-index="' + (l-1) + '">' + (_l < 10 ? '0' + _l : _l) + '</li>';
                          }
						  //alert(_this.getSelected(_this.scroll['DD']) + " " + day_index);
                          _this.$('#' + domId['DD'] + ' ul').innerHTML = li;
                          
						  
						 // _this.scroll['DD'].refresh();
						  _this.scroll['DD'].once('refresh', function () {
							 this.wheelTo(day_index);
							 //_this.scroll['DD'].refresh();
															
						  });
						  _this.scroll['DD'].wheelTo(day_index);
                      }
                  }
				  var m = -1;
				  if("rolldate-month" == that.wrapper.id) m = (parseInt(_this.getSelected(that)) % 12) + 84 - 1;
				  else if(that.wrapper.id == "rolldate-hour") m = (parseInt(_this.getSelected(that)) % 24) + 72;
				  else if(that.wrapper.id == "rolldate-min" || that.wrapper.id=="rolldate-sec") m = (parseInt(_this.getSelected(that)) % 60) + 60 ;
				  else if("rolldate-day" == that.wrapper.id) {
					    
						var _day = _this.bissextile(_this.getSelected(_this.scroll['YYYY']), _this.getSelected(_this.scroll['MM']));
						m = (parseInt(_this.getSelected(that)) % _day) + _day * 3 - 1;
						//alert(m);
						
					}
          console.log("_this.getSelected(that):==="+_this.getSelected(that));
				  if(m != -1) that.wheelTo(m);
              });
          };
      ...
      getSelected: function getSelected(scroll) {
          if(this.config.format.includes("zone")){
            return this.$('#' + scroll.wrapper.id + ' li', 1)[scroll.getSelectedIndex()].innerText;
          }else{
